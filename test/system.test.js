...
describe("Lottery contract", function() {
    ...
    // Creating the instance and contract info for the candy token contract
    let candyInstance;
    ...
    beforeEach(async () => {
        ...
        candyInstance = await mock_erc20Contract.deploy(
            lotto.buy.candy,
        );
        ...
        // Making sure the lottery has some candy
        await candyInstance.mint(
            lotteryInstance.address,
    ...
    describe("Buying tickets tests", function() {
        ...
        it("Batch buying 1 tickets", async function() {
            ...
            await candyInstance.connect(owner).approve(
                lotteryInstance.address,
                price
            );
            // Batch buying tokens
         ...
         */
        it("Invalid candy transfer", async function() {
            // Getting the price to buy
            let price = await lotteryInstance.costToBuyTickets(
                1,
                10
    ...
    describe("Claiming tickets tests", function() {
        beforeEach( async () => {
            ...
            // Sending the buyer the needed amount of candy
            await candyInstance.connect(buyer).mint(
                buyer.address,
                prices[2]
            );
            // Approving lotto to spend cost
            await candyInstance.connect(buyer).approve(
                lotteryInstance.address,
                prices[2]
            );
            // Generating chosen numbers for buy
        ...
        it("Claiming winning numbers (4 (all) match)", async function() {
            ...
            // Sending the buyer the needed amount of candy
            await candyInstance.connect(owner).transfer(
                buyer.address,
                prices[2]
            );
            // Approving lotto to spend cost
            ...
            let buyerCandyBalanceBefore = await candyInstance.balanceOf(buyer.address);
            ...
            let buyerCandyBalanceAfter = await candyInstance.balanceOf(buyer.address);
            // Tests
            assert.equal(
                buyerCandyBalanceBefore.toString(),
                0,
                "Buyer has candy balance before claiming"
            );
            assert.equal(
                buyerCandyBalanceAfter.toString(),
    ...
    describe("Upgrade functionality tests", function() {
        ...
        it("Withdraw excess candy", async function() {
            let ownerCandyBalance = await candyInstance.balanceOf(owner.address);
            let lotteryCandyBalance = await candyInstance.balanceOf(lotteryInstance.address);
            await lotteryInstance.connect(owner).withdrawCandy(lotteryCandyBalance);
            let ownerCandyBalanceAfter = await candyInstance.balanceOf(owner.address);
            let lotteryCandyBalanceAfter = await candyInstance.balanceOf(lotteryInstance.address);
            // Tests
            assert.notEqual(
                ownerCandyBalance.toString(),
                ownerCandyBalanceAfter.toString(),
                "Owner candy balance did not change"
            );
            assert.notEqual(
                lotteryCandyBalance.toString(),
                lotteryCandyBalanceAfter.toString(),
        ...
        it("Invalid withdraw excess candy (non owner)", async function() {
            let lotteryCandyBalance = await candyInstance.balanceOf(lotteryInstance.address);
            await expect(
                lotteryInstance.connect(buyer).withdrawCandy(lotteryCandyBalance)
            ).to.be.revertedWith(lotto.errors.invalid_admin);
        });
    });
    ...
    describe("View function tests", function() {
        ...
        it("Get Lotto Info", async function() {
            ...
            assert.equal(
                lottoInfo.prizePoolInCandy.toString(),
