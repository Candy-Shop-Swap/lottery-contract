...
contract Lottery is Ownable, Initializable, Testable {
    ...
    // State variables 
    // Instance of Candy token (collateral currency for lotto)
    IERC20 internal candy_;
    // Storing of the NFT
    ...
    struct LottoInfo {
        ...
        uint256 prizePoolInCandy;    // The amount of candy for prize money
        uint256 costPerTicket;      // Cost per ticket in $candy
     ...
     * @param   _prizePoolInCandy The amount of Candy available to win in this 
     *          lottery.
        ...
        returns(uint256 lotteryId)
    {
        ...
        require(
            _prizePoolInCandy != 0 && _costPerTicket != 0,
    ...
    function withdrawCandy(uint256 _amount) external onlyOwner() {
        candy_.transfer(
            msg.sender, 
            _amount
        );
        ...
        notContract()
    {
        ...
        // Transfers the required candy to this contract
        candy_.transferFrom(
            msg.sender, 
            address(this), 
            costWithDiscount
    ...
    function claimReward(uint256 _lotteryId, uint256 _tokenId) external notContract() {
        ...
        allLotteries_[_lotteryId].prizePoolInCandy = allLotteries_[_lotteryId].prizePoolInCandy.sub(prizeAmount);
        // Transfering the user their winnings
        candy_.safeTransfer(address(msg.sender), prizeAmount);
    }
        ...
        notContract()
    {
        ...
        candy_.safeTransfer(address(msg.sender), totalPrize);
    }
     ...
     * @return  uint256: The prize amount in candy the user is entitled to 
     */
        ...
        returns(uint256) 
    {
        ...
        prize = allLotteries_[_lotteryId].prizePoolInCandy.mul(perOfPool);
